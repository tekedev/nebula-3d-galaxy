...
export const PromptBox: React.FC<PromptBoxProps> = ({ isLocked = false }) => {
  const [copied, setCopied] = useState(false);

  const promptText = `Prompt to recreate this landing page:
...
Add smooth scroll nav and page transitions.`;
  ...
  return (
    <div className="relative w-full max-w-[720px] mx-auto bg-surface border border-stroke rounded-2xl overflow-hidden mt-8 shadow-2xl shadow-black/60 select-none">
      {/* Box Header */}
      <div className="flex items-center justify-between border-b border-stroke bg-black/40 px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-muted font-mono uppercase tracking-[0.15em] font-semibold">
        ...
        {!isLocked && (
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 rounded-lg bg-white/[0.04] border border-white/5 hover:bg-white/[0.08] hover:border-white/10 px-3 py-1.5 text-xs text-text-primary transition duration-200 cursor-pointer active:scale-95"
          >
            {copied ? (
              <>
      ...
      {/* Box Content */}
      <div className={`p-4 md:p-6 overflow-y-auto max-h-[350px] font-mono text-[11px] sm:text-xs text-left leading-relaxed text-white/80 bg-[#07070f] ${isLocked ? 'blur-sm select-none pointer-events-none opacity-40' : 'select-all'}`}>
        <pre className="whitespace-pre-wrap">
          {isLocked 
            ? promptText.split('\n').map(line => '•'.repeat(line.length || 5)).join('\n')
            : promptText}
        </pre>
      </div>
    </div>
